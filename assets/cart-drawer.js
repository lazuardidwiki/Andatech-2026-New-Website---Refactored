...
if (!customElements.get('cart-drawer')) {
  class CartDrawer extends SideDrawer {
    ...
    open(opener, elementToFocus) {
      // Lock the page behind the drawer. Plain overflow:hidden on <html>+<body>
      // — the position:fixed trick breaks inner scroll on iOS Safari.
      CartDrawer.lockBodyScroll();
    ...
    close(callback) {
      ...
      CartDrawer.unlockBodyScroll();
    }

    static lockBodyScroll() {
      if (document.documentElement.dataset.cartLocked === '1') return;
      document.documentElement.dataset.cartLocked = '1';
    ...
    static unlockBodyScroll() {
      if (document.documentElement.dataset.cartLocked !== '1') return;
      delete document.documentElement.dataset.cartLocked;
